...
class TestOrphanedRunReconcile:
    def test_force_reconcile_marks_running_run_failed_immediately(self, stores):
        _, rs, ns = stores
        run = rs.create_run({"task_id": "t1", "runtime": "local"})
        rid = run["id"]
        rs.transition_run(rid, "running")

        ne = ns.create_node_execution({"run_id": rid, "node_key": "builder"})
        ns.transition_node(ne["id"], "running")
        ns.transition_node(ne["id"], "failed")

        with patch.object(server, "get_run_store", return_value=rs), patch.object(server, "get_node_execution_store", return_value=ns):
            reconciled = server._reconcile_orphaned_running_run(rid, now=time.time(), force=True)

        assert reconciled is not None
        assert reconciled["status"] == "failed"
        assert reconciled["active_node_execution_ids"] == []
        assert reconciled["current_node_execution_id"] == ""
...
class TestTimeoutConstants:
    ...
    def test_default_timeout_constants_exist(self):
        assert server.DEFAULT_NODE_TIMEOUT_S == 960
        assert server.DEFAULT_RUN_TIMEOUT_S == 3600
        assert server.WATCHDOG_INTERVAL_S == 30

    def test_builder_watchdog_timeout_floor_tracks_multifile_cap(self, monkeypatch):
        ...
        assert server._builder_watchdog_timeout_floor_seconds() == 1890

    def test_node_timeout_limit_raises_builder_above_old_stale_hint(self, monkeypatch):
        monkeypatch.setenv("EVERMIND_BUILDER_DIRECT_MULTIFILE_MAX_TIMEOUT_SEC", "1800")
        monkeypatch.setenv("EVERMIND_WATCHDOG_TIMEOUT_GRACE_SEC", "90")
        limit = server._node_timeout_limit_seconds({
            "node_key": "builder",
            "timeout_seconds": 1020,
        })
        assert limit == 1890

    def test_node_timeout_limit_preserves_non_builder_explicit_timeout(self):
        limit = server._node_timeout_limit_seconds({
            "node_key": "planner",
            "timeout_seconds": 42,
        })
...
class TestRunReconciliation:
    def test_list_runs_reconciles_stale_running_failure(self, ws_env):
        _task_id, run_id, node_ids = _create_run_with_nodes(
            ["analyst", "builder1", "builder2", "reviewer"],
            runtime="local",
        )
        ...
        ns.transition_node(node_ids[0], "running")
        ns.transition_node(node_ids[0], "passed")
        ns.transition_node(node_ids[1], "running")
        ns.transition_node(node_ids[1], "passed")
        ns.transition_node(node_ids[2], "running")
        ns.transition_node(node_ids[2], "failed")

        stale_ts = time.time() - (server.ORPHANED_RUNNING_RUN_STALE_S + 30)
        rs.update_run(run_id, {
            "status": "running",
            "updated_at": stale_ts,
            "started_at": stale_ts - 60,
            "current_node_execution_id": "",
            "active_node_execution_ids": [],
        })
        for node_id in node_ids:
            ns.update_node_execution(node_id, {"updated_at": stale_ts, "ended_at": stale_ts})

        with TestClient(server.app) as client:
            resp = client.get("/api/runs")
            assert resp.status_code == 200
            run = next(item for item in resp.json()["runs"] if item["id"] == run_id)
            assert run["status"] == "failed"
            assert run["active_node_execution_ids"] == []
            assert run["current_node_execution_id"] == ""

    def test_get_run_reconciles_stale_running_success(self, ws_env):
        _task_id, run_id, node_ids = _create_run_with_nodes(
            ["analyst", "builder1"],
            runtime="local",
        )
