...
def test_run_goal_pro_complex_goal_uses_parallel_builder_quality_path(ws_env, monkeypatch):
    ...
    with TestClient(server.app) as client:
        with client.websocket_connect("/ws") as ui_ws:
            ...
            assert nodes[3]["depends_on_keys"] == ["analyst", "uidesign"]
            assert nodes[4]["depends_on_keys"] == ["analyst", "uidesign"]
            assert nodes[5]["depends_on_keys"] == ["builder1", "builder2", "scribe"]
            assert observed["canonical_context"]["node_executions"][3]["depends_on_keys"] == ["analyst", "uidesign"]


def test_run_goal_pro_voxel_game_skips_asset_pipeline(ws_env, monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    observed = {}
    ...
    with TestClient(server.app) as client:
        with client.websocket_connect("/ws") as ui_ws:
            ...
            assert [node["node_key"] for node in nodes] == [
                "analyst", "builder1", "builder2", "reviewer", "deployer", "tester", "debugger"
            ]
            assert all(node["node_key"] not in {"imagegen", "spritesheet", "assetimport"} for node in nodes)
