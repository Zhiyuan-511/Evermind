    ...
    async def call(
        ...
        **kwargs,
        ...
        try:
            ...
            hard_timeout = min(endpoint.timeout + 15, MAX_TIMEOUT_SECS)
            response = await asyncio.wait_for(
                asyncio.to_thread(litellm.completion, **call_kwargs),
            ...
            for retry in range(endpoint.max_retries):
                try:
                    ...
                    retry_timeout = min(endpoint.timeout + 15, MAX_TIMEOUT_SECS)
                    response = await asyncio.wait_for(
                        asyncio.to_thread(litellm.completion, **call_kwargs),
