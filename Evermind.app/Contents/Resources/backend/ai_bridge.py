...
AGENT_PRESETS = {
    ...
    "polisher": {
        "instructions": (
            ...
            "4. Replace visual placeholders aggressively: blank media slots, plain gradient stand-ins, empty map blocks, and generic visual divs must become real premium visuals\n"
            ...
            "- By your second tool turn, you must begin file_ops write unless a write just failed and needs one corrective read\n"
            "- Edit HTML/CSS/JS in place with file_ops write, including routed HTML pages under /tmp/evermind_output/\n"
            "- Fill image/visual placeholders with real visual content: high-quality remote editorial images when appropriate, or rich CSS/SVG compositions when photo sourcing is unnecessary\n"
...
class AIBridge:
    ...
    def _max_tool_iterations_for_node(self, node_type: str) -> int:
        ...
        if normalized_node_type == "analyst":
            return self._read_int_env("EVERMIND_ANALYST_MAX_TOOL_ITERS", 3, 2, 6)
    ...
    def _polisher_non_write_followup_prompt(
        ...
        return (
            ...
            "such as styles.css, app.js, index.html, or the affected routed pages. "
            f"{page_hint}"
            "Patch the current files in place instead of re-reading the whole site. "
            "Replace blank media placeholders, gradient-only image stand-ins, empty map/location blocks, [Collection Image]-style placeholder copy, and thin motion with finished premium visuals and interactions now. "
                    ...
                    if fn_name in {"browser", "browser_use"} and self._desktop_qa_browser_suppressed(node_type, input_data):
                        ...
                        if action == "write":
                            result = {
                                ...
                                "data": {},
                                "error": (
                                    f"{normalized_node_type.capitalize()} nodes must NOT write code files. "
                                    "Produce your research report / design brief as plain text content in your response, "
                                    "using the required XML tags. Do NOT use file_ops write."
                                ),
                                "artifacts": [],
                            }
                            logger.warning(
                    ...
                    if fn_name in {"browser", "browser_use"} and self._desktop_qa_browser_suppressed(node_type, input_data):
                        ...
                        if block_action == "write":
                            result = {
                                ...
                                "error": (
                                    f"{normalized_node_type.capitalize()} nodes must NOT write code files. "
                                    "Produce your output as plain text content using XML tags."
                                ),
                                "artifacts": [],
                            }
                            logger.warning(
