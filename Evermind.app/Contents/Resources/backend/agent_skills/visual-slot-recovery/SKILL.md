...
Never ship blank media zones.

- Audit every obvious visual container before finishing: classes or sections named `image`, `photo`, `visual`, `media`, `gallery`, `map`, `cover`, `showcase`, `story`, `hero`, or `placeholder`.
- Replace unfinished visual slots with one of these finished outcomes:
...
- When practical, rename or restyle placeholder-flavored classes so the shipped DOM no longer reads like unfinished scaffolding.

Before finishing, verify:
- no empty image wrappers
- no `img` tags with blank or fake `src`
- no placeholder copy left inside visual blocks
- no empty map/location modules
- no visual block that looks like a wireframe or asset TODO
