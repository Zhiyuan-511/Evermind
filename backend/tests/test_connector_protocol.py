...
class ConnectorProtocolWsTestCase(unittest.TestCase):
    ...
    def _create_task_run_node(self, *, run_id: str, node_id: str, node_key: str = "builder"):
        created_task = asyncio.run(server.create_task({"title": f"Task for {run_id}"}))
        task_id = created_task["task"]["id"]
        run = asyncio.run(server.create_run({"id": run_id, "task_id": task_id}))
    ...
    def test_run_complete_projects_task_review_and_enriches_broadcast_payload(self):
