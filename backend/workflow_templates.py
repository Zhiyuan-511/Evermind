...
BUILT_IN_TEMPLATES: Dict[str, Dict] = {
    "simple": {
        ...
        # v7.8: tester removed; reviewer (when present)
...
_PRO_ASSET_HEAVY_RE = re.compile(
    ...
    # v7.3.6: include "建模" / "modeling" / "精灵图" /
    ...
    # v7.43: observed in run_4f4e5f0766b0 — user goal
...
def pro_template_profile(goal: str = "") -> Dict[str, Any]:
    ...
    if asset_heavy and task_type == "game":
        return {
            ...
            # v7.1i: was 3 (forced game>=3 branch in
    ...
    if task_type == "game":
        return {
            ...
            "include_polisher": False,
            "parallel_builders": True,
            # v7.1i: same fix as asset_heavy_game above —
    ...
    if needs_uidesign and needs_scribe:
        ...
        # v7.55: add third trigger — single-page but
...
def _build_pro_template(goal: str = "") -> Dict[str, Any]:
    ...
    # v7.1i: WAS `task_type != "website" and builder_count <= 2`
    ...
    # v6.3: every pro branch gets a conditional patcher
    ...
    if _has_reviewer and not _has_patcher:
        # v6.4: patcher is a canvas-parallel sibling of
...
def _build_ultra_template_for_goal(goal: str) -> Dict:
    """v7.1i: adapt the 4-builder labels to task type.
