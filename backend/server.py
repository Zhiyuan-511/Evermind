...
def _auto_chain_next_node(run_id: str) -> "list[str] | str | None":
    ...
    def _can_degrade(node_key: str) -> bool:
        if not _is_join_node(node_key):
            return False
        run_snapshot = get_run_store().get_run(run_id)
        run_started_at = float((run_snapshot or {}).get("started_at") or (run_snapshot or {}).get("created_at") or 0.0)
        _task_id, preview_file = latest_preview_artifact(OUTPUT_DIR)
        if not preview_file or not preview_file.exists():
            return False
        if run_started_at > 0:
            try:
                return preview_file.stat().st_mtime >= max(run_started_at - 2.0, 0.0)
            except Exception:
                return False
        return True
    ...
    while True:
        ...
        for ne in nes:
            ...
            succeeded = [d for d in deps if key_status.get(d) in SUCCESS_STATUSES]
            if failed_or_terminal and succeeded and _can_degrade(ne.get("node_key", "")):
                ready.append(ne["id"])
                continue
            ...
            for blocked_id in blocked:
                ...
                if transitioned.get("success"):
                    ns.update_node_execution(blocked_id, {
                        "error_message": "Blocked by failed dependencies; no safe preview artifact available.",
                    })
            continue
    ...
    try:
        while True:
            ...
            if msg_type == "ping":
                ...
                requested_model = str(model or "gpt-5.4")
                ...
                try:
                    ...
                    if valid_custom_nodes:
                        ...
                        tpl = get_template(difficulty)
                        template_id = difficulty
                        nodes_def = tpl["nodes"] if tpl else get_template("standard")["nodes"]
                        if not tpl:
                    ...
                    for node_def in nodes_def:
                        ...
                        ne = nes_store.create_node_execution({
                            ...
                            "loaded_skills": resolve_skill_names_for_goal(node_key, str(input_summary or goal)),
                        })
                        created_nes.append(ne)
                    ne_ids = [ne["id"] for ne in created_nes]
                ...
                if run_success and preview_url:
                    try:
                        ...
                        await broadcast_connector_event({
                            ...
                            "output_dir": str(Path("/tmp/evermind_output")),
                            "final": True,
                        })
